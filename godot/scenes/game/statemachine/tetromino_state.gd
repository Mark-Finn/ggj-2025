...
static var _next_down_time: float = 0
...
func process(delta: float, game_clock: Timer) -> void:
	...
	var slam_pressed = Input.is_action_pressed("space")
	var down_pressed = Input.is_action_pressed("down")
	
	if left_pressed && right_pressed:
		return
	elif down_pressed && (_next_down_time < 0 || Input.is_action_just_pressed("down")):
		if tetromino.try_move(Vector2(0, -2)):
			_next_down_time = HOLD_MOVE_PERIOD
